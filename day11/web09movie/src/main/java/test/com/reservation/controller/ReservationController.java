 ...
 */
@WebServlet({"/re_selectAll.do", "/re_insert.do","/re_insertDetail.do", "/re_insertOK.do", "/re_update.do", 
...
//			request.getRequestDispatcher("reservation/selectAll.jsp").forward(request, response);
		...
		}else if (sPath.compareTo("/re_insertDetail.do") == 0) {

			request.getRequestDispatcher("reservation/insertDetail.jsp").forward(request, response);
...
//			String movie_num = request.getParameter("movie_num");
			...
			movie_date = movie_date.replace("T", " ");
